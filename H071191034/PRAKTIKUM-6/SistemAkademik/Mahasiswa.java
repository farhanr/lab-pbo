public class Mahasiswa implements ProgramStudi{

    private String name;
    private String prodi;
    private String nim;
    private double uas;
    private double uts;

    public Mahasiswa (String name, String nim, String prodi, double uas, double uts){
        this.name = name;
        this.nim = nim;
        this.uts = uts;
        this.uas = uas;
        this.prodi = prodi;
    }

    public String getName() {
        return this.name;
    }
    
    public String getProdi() {
    	return this.prodi;
    }
    
    public String getNim() {
    	return this.nim;
    }
    
    public double getUas() {
    	return this.uas;
    }
    
    public double getUts() {
    	return this.uts;
    }

    @Override
    public void prodi(String value, int data) {
        System.out.println("SKS           : " + data);
        System.out.println("Dosen         : " + value);
    }
    
    public double score(double uts, double uas){
        return (uas+uts)/2;
    }
    
    public void info(){
        System.out.println("\nSistem Akademik");
        System.out.println("-----------------------------------------------");
        System.out.println("NIM           : " + nim);
        System.out.println("Nama          : " + name);
        System.out.println("Mata Kuliah   : " + prodi);
    }
    public void nilai(){
        System.out.println("Nilai UTS     : " + uts);
        System.out.println("Nilai UAS     : " + uas);
        System.out.println("Nilai Akhir   : " + score(uts, uas));
        if (score(uts, uas) > 90){
            System.out.println("Angka Mutu    : A" );
        } else if (score(uts, uas) > 80){
            System.out.println("Angka Mutu    : B");
        } else if (score(uts, uas) > 70){
            System.out.println("Angka Mutu    : C");
        } else if (score(uts, uas) > 60){
            System.out.println("Angka Mutu    : D");
        } else{
            System.out.println("Angka Mutu    : E");
        }
        System.out.println("-----------------------------------------------");

    }

}